...
const cardData = [
  { color: '#060010', title: '$12,480.00', description: '↑ 8.2% this month',    label: 'Total Balance',  icon: '💰' },
  { color: '#060010', title: '$5,200.00',  description: '↑ 12% vs last month',  label: 'Total Income',   icon: '📈' },
  { color: '#060010', title: '$2,340.00',  description: '↑ 3.1% vs last month', label: 'Total Expenses', icon: '📉' },
  { color: '#060010', title: '$2,860.00',  description: '↑ 18% this month',     label: 'Total Savings',  icon: '🎯' },
]
...
const ParticleCard = ({
  ...
  return (
    <div ref={cardRef} className="card card--border-glow" style={{ ...style, position: 'relative', overflow: 'hidden' }}>
      {children}
    </div>
  );
};
  ...
  const CardInner = ({ card }) => (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', fontWeight: '500' }}>{card.label}</span>
        <span style={{ fontSize: '20px' }}>{card.icon}</span>
      </div>
      <div style={{ marginTop: 'auto' }}>
        <h3 style={{ fontWeight: '700', fontSize: '18px', margin: '0 0 6px 0', color: 'white', overflow: textAutoHide ? 'hidden' : 'visible', textOverflow: 'ellipsis', whiteSpace: textAutoHide ? 'nowrap' : 'normal' }}>
          {card.title}
        </h3>
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ padding: '12px', width: '100%', maxWidth: '860px', userSelect: 'none', position: 'relative' }}
      >
        <div
          ref={gridRef}
          style={{
        ...
        >
          {cardData.map((card, index) => (
            enableStars ? (
              ...
              <div key={index} className="card card--border-glow" style={cardStyle(index)}>
                <CardInner card={card} />
              </div>
            )
          ))}
